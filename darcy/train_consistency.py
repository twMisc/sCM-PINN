...
from utils import FDM_Darcy_loss, discrete_a, scale_back_u, scale_back_a, plot_result, load_data, consistency_sample_cm
...
MY_PATH_TO_TEACHER = './darcy_redo_output/sCM/diffusion/model_epoch/model_epoch_10.pth'
...
def train_loop(
    ...
    for epoch in range(epochs):
        ...
        for batch in progress_bar:
            ...
            #### PDE loss (might moved to other scripts later)
            if use_pde_loss:
                # # stage 2 physics-informed loss
                ...
                loss_rand_res = FDM_Darcy_loss(u_samples,
                                            a_samples_discrete,
                                            output_mask=False)
                loss_rand = torch.square(loss_rand_res)
                loss_rand = loss_rand.mean()
                loss = loss + 1e-2*loss_rand  # lambda for physics-informed loss
...
@click.option('--save_path', default='./darcy_redo_output/sCM/consistency', help='Path to save model checkpoints and samples.')
