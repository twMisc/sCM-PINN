...
from utils import FDM_Darcy_loss, Energy_Darcy_loss, discrete_a
    ...
    for epoch in range(epochs):
        ...
        for batch in progress_bar:
            ...
            with torch.amp.autocast('cuda', enabled=use_amp, dtype=torch.bfloat16):
                ...
                ## add the Ritz loss here
                # we want to get a x0 prediction from v_t prediction
                # images is the exact x0
                if use_ritz:
                    pred_x0 = torch.cos(t) * x_t - torch.sin(t) * pred_v_t
                    ...
                    energy_ritz = Energy_Darcy_loss(u_samples,a_samples_discrete)

                    loss += 1.0 * energy_ritz
...
@click.option('--save_interval', default=500, help='Interval (in steps) to save model checkpoints and samples.')
@click.option('--save_path', default='./sCM/diffusion', help='Path to save model checkpoints and samples.')
@click.option('--seed', help='Optional random seed for reproducibility', default=None, type=int)
@click.option('--amp/--no-amp', default=False, help='Enable/disable AMP (bfloat16 autocast).')
@click.option('--ema/--no-ema', default=False, help='Enable/disable EMA model tracking.')
@click.option('--ritz/--no-ritz', default=False, help='Enable/disable Ritz loss.')
def main(device, epochs, batch_size, lr, save_interval, save_path, seed, amp, ema, ritz):
    if seed is not None:
        np.random.seed(seed)
