...
from utils import load_test_data, rescale_a, sample_dpm_solver, consistency_sample_cm, scale_back_a, scale_back_u, rescale_u, rescale_a, helmholtz_loss, calculate_h1_error, calculate_semi_norm
...
def main(device, batch_size, total_samples, model_path, save_path, save_name, seed, model_type, network_type, num_steps, cm_steps, use_pde_mask, problem_type, recon_mask_percent, figure_dir, figure_name, network_name, mask_mse_eval, zeta_obs_a, zeta_obs_u, zeta_pde, rho):
    ...
    with torch.no_grad():
        for i in range(0, total_samples, batch_size):
            ...
            if model_type == 'diffpde':
                ...
                if cm_steps > 2:
                    pred = consistency_sample_cm(model, sigma_data=0.5, device=device, shape=(batch_size, 2, 128, 128), schedule='power', return_intermediates=False, use_seeded_z=False, x_obs=x_obs, mask=mask, n_steps=cm_steps, sigma_min=1e-8)
                else:
                    pred = consistency_sample_cm(model, sigma_data=0.5, device=device, shape=(batch_size, 2, 128, 128), t_list=times, return_intermediates=False, use_seeded_z=False, x_obs=x_obs, mask=mask, sigma_min=1e-8)
    ...
    if problem_type == 'reconstruction' and mask_mse_eval:
        ...
        print(f"Rel L2 a: {err_a:.4f}, Rel L2 u: {err_u:.4f}")
        ...
        # --- NEW: Semi-norm Error Calculation ---
        semi_h1_errors = []
        rel_semi_h1_errors = []
        semi_h1_norms = []
        for j in range(gen_u_samples.shape[0]):
            semi_h1, rel_semi_h1 = calculate_semi_norm(gen_u_samples[j], u_data[j], h=1.0/127)
            semi_h1_norm, _ = calculate_semi_norm(gen_u_samples[j], np.zeros_like(gen_u_samples[j]), h=1.0/127)
            semi_h1_norms.append(semi_h1_norm)
            semi_h1_errors.append(semi_h1)
            rel_semi_h1_errors.append(rel_semi_h1)
        mean_semi_h1 = np.mean(semi_h1_errors)
        mean_rel_semi_h1 = np.mean(rel_semi_h1_errors)
        mean_semi_h1_norm = np.mean(semi_h1_norms)
        print(f"Mean Semi-norm Error: {mean_semi_h1:.4f}, Mean Rel Semi-norm Error: {mean_rel_semi_h1:.4f}, Mean Semi-norm: {mean_semi_h1_norm:.4f}")

        # add inverse problem specific metrics. Relative H^1 error for 'a' (only for inverse problem)
        if problem_type == 'inverse':
            h1_errors_a = []
            rel_h1_errors_a = []
            h1_norms_a = []
            for j in range(gen_a_samples.shape[0]):
                h1_a, rel_h1_a = calculate_h1_error(gen_a_samples[j], a_data[j], h=1.0/127)
                h1_norm_a, _ = calculate_h1_error(gen_a_samples[j], np.zeros_like(gen_a_samples[j]), h=1.0/127)
                h1_norms_a.append(h1_norm_a)
                h1_errors_a.append(h1_a)
                rel_h1_errors_a.append(rel_h1_a)
            mean_h1_a = np.mean(h1_errors_a)
            mean_rel_h1_a = np.mean(rel_h1_errors_a)
            mean_h1_norm_a = np.mean(h1_norms_a)
            print(f"Mean H1 Error for a: {mean_h1_a:.4f}, Mean Rel H1 Error for a: {mean_rel_h1_a:.4f}, Mean H1 Norm for a: {mean_h1_norm_a:.4f}")
    ...
    pde_residual = helmholtz_loss(torch.tensor(gen_u_samples), torch.tensor(gen_a_samples)).numpy()

    pde_error = np.mean(pde_residual)
    ...
    results = {
        ...
        'mean_semi_h1_error': mean_semi_h1,
        'mean_rel_semi_h1_error': mean_rel_semi_h1,
        'mean_semi_h1_norm': mean_semi_h1_norm
    }
    ...
    # np.savez(os.path.join(save_path, f"generated_samples_{model_type}_{network_type}_{problem_type}.npz"), gen_a=gen_a_samples, gen_u=gen_u_samples, real_a=a_data, real_u=u_data)
