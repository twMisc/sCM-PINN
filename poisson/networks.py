...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        conditioning_mode="adn",   # <--- NEW: "adagn", "adn", "additive"
        ...
        self.conditioning_mode = conditioning_mode.lower()
        ...
        # affine outputs depend on mode
        if self.conditioning_mode in ["adagn", "adn"]:
    ...
    def forward(self, x, emb):
        ...
        if self.num_heads:
            q, k, v = self.qkv(self.norm2(x)).reshape(
                x.shape[0] * self.num_heads,
                x.shape[1] // self.num_heads,
                3, -1
            ).unbind(2)

            w = self.attention_op(q, k)
...
class SongUNet(torch.nn.Module):
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None, return_emb=False):
        # Mapping.
...
class SongUNetEncoder(nn.Module):
    def __init__(self, song_unet: SongUNet):
        super().__init__()
        self.map_noise = song_unet.map_noise
        self.map_label = song_unet.map_label
        self.map_augment = song_unet.map_augment
        self.map_layer0 = song_unet.map_layer0
        self.map_layer1 = song_unet.map_layer1
        self.enc = song_unet.enc
        self.label_dropout = song_unet.label_dropout
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None):
        ...
        for name, block in self.enc.items():
            if 'aux_down' in name:
                ...
                x = block(x, emb) if hasattr(block, 'forward') and 'emb' in block.forward.__code__.co_varnames else block(x)
                skips.append(x)
        return x, skips, emb

class SongUNetDecoder(nn.Module):
    def __init__(self, dec: nn.ModuleDict):
        super().__init__()
        self.dec = dec
...
class SplitSongUNet(nn.Module):
    def __init__(self, encoder, decoder_frozen, decoder_trainable, frozen=True):
        super().__init__()
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None, return_emb=False, output_frozen_u=False):
        x, skips, emb = self.encoder(x, noise_labels, class_labels, augment_labels)
        
        if self.frozen:
            # Freeze the decoder
            with torch.no_grad():
                frozen_out = self.decoder_frozen(x.clone(), skips.copy(), emb)
        else:
            # Unfreeze the decoder
            frozen_out = self.decoder_frozen(x.clone(), skips.copy(), emb)

        trainable_out = self.decoder_trainable(x, skips, emb)
        if output_frozen_u:
            return torch.cat([frozen_out[:, 0:1, ...], trainable_out[:, 1:, ...]], dim=1), frozen_out[:, 1:2, ...]
        if not return_emb:
            return torch.cat([frozen_out[:, 0:1, ...], trainable_out[:, 1:, ...]], dim=1)
        else:
            return torch.cat([frozen_out[:, 0:1, ...], trainable_out[:, 1:, ...]], dim=1), emb
