...
def consistency_sample_cm(
    ...
    *,
    ...
    if t_list is not None:
        t_all = torch.as_tensor(t_list, dtype=dtype, device=device)
    else:
        i = torch.arange(n_steps + 1, device=device, dtype=dtype)
        if schedule == "karras":
    ...
    if z is None:
        if use_seeded_z:
            z = torch.randn(shape, generator=torch.Generator(device=device).manual_seed(123), device=device, dtype=dtype)
        else:
    ...
    for k in range(1, len(t_all)):
        if use_seeded_z:
            z = torch.randn(shape, generator=torch.Generator(device=device).manual_seed(123+k), device=device, dtype=dtype)
        else:
            z = torch.randn(shape, device=device, dtype=dtype)
        t = t_all[k].expand(B)
...
def poisson_loss(u, a, return_residual=False):
    """
    Compute the Poisson loss given u and a.
