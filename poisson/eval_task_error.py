...
from utils import load_test_data, rescale_a, sample_dpm_solver, consistency_sample_cm, scale_back_a, scale_back_u, rescale_u, rescale_a, poisson_loss, calculate_h1_error, calculate_semi_norm
...
def main(device, batch_size, total_samples, model_path, save_path, save_name, seed, model_type, network_type, num_steps, cm_steps, use_pde_mask, problem_type, recon_mask_percent, figure_dir, figure_name, network_name, mask_mse_eval, zeta_obs_a, zeta_obs_u, zeta_pde, rho):
    ...
    if problem_type == 'reconstruction' and mask_mse_eval:
        ...
        # --- New: Semi-H1 Error Calculation ---
        # We calculate the semi-H1 error which only considers the gradient part (no L2 term)
        semi_h1_errors = []
        rel_semi_h1_errors = []
        semi_h1_norms = []
        ...
        print(f"Mean Semi-H1 Error: {mean_semi_h1:.4f}, Mean Rel Semi-H1 Error: {mean_rel_semi_h1:.4f}, Mean Semi-H1 Norm: {mean_semi_h1_norm:.4f}")
    ...
    pde_residual = poisson_loss(torch.tensor(gen_u_samples), torch.tensor(gen_a_samples)).numpy()
